...
# from models.utils_helper import get_distance
...
class Rewrite:
    ...
    def match(self, area, name, addr, blng, blat, slng, slat, info, soso_st, soso_ba, soso_district):
        ...
        if area in self.conf.ba:
            for ba_item in self.conf.ba[area]:
                if ba_item[0] in name and 5000 > utils.get_distance(slng, slat, ba_item[1], ba_item[2]):
                    info['ba'] = ba_item[0]
                    return 50
        ...
        # name中已有路、街、道用区改写
        if re.search(u'路|街|道', name):
            info['district'] = soso_district
            return 75 if soso_district and soso_district not in name and not any(region in name for region in self.conf.region_list) else 74
    ...
    def fill(self, rwtype, info, area, name, addr, blng, blat, slng, slat):
        if rwtype in [10, 20, 40, 50, 60, 70, 74, 80]:
            addr = ""

        if rwtype == 75 and 'district' in info:
            addr = info['district']

        if rwtype == 90 and 'ba' in info:
            addr = info['ba']
